  ...
  ngOnChanges() {
    if (this.data?.slug == this.slug) {
      ...
      if (this.data?.content?.offer_banner_1?.banner_1?.status) {
        this.banners = [
          ...this.banners,
          this.data?.content?.offer_banner_1?.banner_1,
        ];
      }
      if (this.data?.content?.offer_banner_1?.banner_2?.status) {
        this.banners = [
          ...this.banners,
          this.data?.content?.offer_banner_1?.banner_2,
        ];
      }
      if (this.data?.content?.offer_banner_1?.banner_3?.status) {
        this.banners = [
          ...this.banners,
          this.data?.content?.offer_banner_1?.banner_3,
        ];
      }

      let categoryIds =
        this.data?.content?.products_list?.categories?.category_ids;

      // Get Products
      let getProducts$;
      if (
        this.data?.content?.products_ids?.length &&
        this.data?.content?.products_list?.products?.status
      ) {
